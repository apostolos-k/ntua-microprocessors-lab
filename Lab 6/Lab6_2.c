...
unsigned char x, y;         // x is 1st digit and y is 2nd digit (in ASCII))
...
unsigned char scan_row(int i) {
    ...
    a = (a << i);
    a = ~a;
    PCA9555_0_write(REG_OUTPUT_1, a);           // Set 0 (inverted logic) to the i-th row
    uint8_t inp = PCA9555_0_read(REG_INPUT_1);  // Read input to check if column is tapped on the i-th row
    return ~(inp | 0x0F);	                    // Keep 4 MSB which are for the column (~inverted logic)
}
...
int scan_keypad_rising_edge() {
    ...
    keypad[0] &= tmp_keypad[0];             // Compare states before and after 10 ms
